...
export default function header() {
  return (
    <header className='z-[999] relative'>
        <motion.div className='fixed top-0 left-1/2 h-[4.5rem] -translate-x-1/2
             ...
             nded-full dark:bg-gray-950 dark:border-black/40 dark:bg-75'
             initial={{y: -100, x: "-50%", opacity: 0}}
             animate={{y: 0, x: "-50%", opacity: 1}}>
        </motion.div>
        <nav className='fixed flex top-[0.15rem] left-1/2 h-12 
                ...
                {links.map(link =>
                    <motion.li className='h-3/4 flex items-center justify-center'
                        ...
                        <Link className='flex w-full items-center 
                            justify-center px-3 py-3 hover:text-gray-950
                            transition'
